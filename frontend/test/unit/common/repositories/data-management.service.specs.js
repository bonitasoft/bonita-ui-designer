describe('DataManagementService', () => {
  ...
  it('should load only real Business Object', () => {
        ...
        data: {
          __schema: {
            types: [
              ...
              { name: 'String', kind: 'SCALAR', description: 'The `String` scalar type represents textual data, represented as UTF-8 character sequences. The String type is most often used by GraphQL to represent free-form human-readable text.' },
              { name: 'com_bonita_model_Order', kind: 'OBJECT', description: null },
              { name: 'Int', kind: 'SCALAR', description: 'The `Int` scalar type represents non-fractional signed whole numeric values. Int can represent values between -(2^31) and 2^31 - 1.' }, { name: 'com_bonita_model_OrderInfo', kind: 'OBJECT', description: null },
    ...
    dataManagementRepo.getDataObjects().then((data) => {
      ...
      expect(data[0].id).toEqual('com_bonita_model_Currency');
      expect(data[0].qualifiedName).toEqual('com.bonita.model.Currency');
      expect(data[0].name).toEqual('Currency');
    });
    $httpBackend.flush();
  });
  ...
  it('should return an empty array when try to load businessObject with business data repository is on error', () => {
    $httpBackend.whenPOST('/bdr').respond(500);
