describe('pbButton', function () {
  ...
  describe('Submit task action', function () {
    ...
    it('should remove LocalStorage entry for the current task URL', function () {
      ...
      var formId = "bonita-form-" + $window.location.href; // id generated by pbSaveButtonWidget
      var savedData = "to be deleted";
      $window.localStorage.setItem( formId, savedData);
      expect($window.localStorage.getItem(formId)).toEqual(savedData);
      ...
      expect($window.localStorage.getItem(formId)).toEqual(null);
    });
  });
